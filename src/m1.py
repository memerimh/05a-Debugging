...
def broken_5(circle, window):
    ...
    """
    circle.attach_to(window)
    square = rg.Square(circle.center, circle.radius * math.sqrt(2))
...
def broken_7(n, point, length, distance_between_lines, window):
    ...
    for k in range(n):
        left = rg.Point(left.x, left.y + (k * distance_between_lines))
        right = rg.Point(right.x, right.y + (k * distance_between_lines))
